...
#### **Motivation Behind Dynamic Programming**

The motivation behind dynamic programming (DP) lies in a single idea—why do something many times when you only need to do it once? Unlike some other algorithms, there isn’t one way to implement dynamic programming. Instead, DP is a method that can be used in a plethora of different problems to reduce runtime.

DP is a form of optimization that breaks large and complex problems into smaller sub-structures/sub-problems. This simplifies the algorithms required and makes the code more understandable. DP consists of 2 main concepts: optimal substructure (the best simplification) and overlapping sub-problems (each substructure links to another). 
...
<details>
<summary>What is the time complexity for the original recursion? What about the one using DP? Why is the DP one preferable, still?</summary>
...
<details>
<summary>Implement a bottom-up DP algorithm that solves the rod problem.</summary>
